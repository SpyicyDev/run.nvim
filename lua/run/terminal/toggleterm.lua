...
function M.run(cmd, opts)
  ...
  if not ok_mod then
    require("run.util").notify("toggleterm.nvim not available; falling back to built-in terminal", vim.log.levels.WARN)
  ...
  if not ok then
    require("run.util").notify(("toggleterm.exec failed: %s"):format(err), vim.log.levels.ERROR)
    return false
  end
  return true
end
